...
void GPIO_toggel(portX_t portName, pinX_t pinNum)
{
	static uint8_t u8_toggel_status=PIN_IS_LOW;
	switch(portName)
	{
		case PORT_0:
			if(u8_toggel_status == PIN_IS_LOW)
			{
				SET_BIT(IOSET0, pinNum);
				u8_toggel_status=PIN_IS_HIGH;
			}
			else if (u8_toggel_status == PIN_IS_HIGH)
			{
				SET_BIT(IOCLR0, pinNum);
				u8_toggel_status=PIN_IS_LOW;
			}
			else
			{
